...
test("semantic-release bootstraps Site Alban at 0.0.1 after green master CI", async () => {
	...
	assert.match(release, /workflow_run:[\s\S]*?branches:\s*\n\s*- master/);
...
test("release bootstrap anchors v0.0.0 before semantic-release workflow changes", async () => {
	...
	const configCommitPosition = release.indexOf(
		"git log --reverse --format=%H -- .releaserc.yaml",
	);
	const tagPosition = release.indexOf('git tag v0.0.0 "${BASELINE_SHA}"');

	assert.ok(
		identityPosition >= 0,
		"release workflow must configure git user.name",
	);
	assert.ok(
		configCommitPosition >= 0,
		"release workflow must locate the first release config commit",
	);
	assert.ok(
		tagPosition >= 0,
		"release workflow must create the technical baseline tag",
	);
	assert.ok(
		identityPosition < tagPosition,
		"git identity must be configured before bootstrap mutations",
	);
	assert.ok(
		configCommitPosition < tagPosition,
		"baseline must be resolved before creating v0.0.0",
	);
	assert.match(
		release,
...
test("release bootstrap creates only the tag ref through the GitHub API", async () => {
	const release = await source(".github/workflows/release.yml");
	const bootstrapStart = release.indexOf(
		"      - name: Bootstrap semantic-release baseline",
	);
	const bootstrapEnd = release.indexOf(
		"      - name: Verify first-release bootstrap version",
	);
...
test("release bootstrap requires the dedicated App token", async () => {
	...
	assert.ok(
		bootstrapStart >= 0,
		"release workflow must contain the bootstrap step",
	);
	assert.ok(
		bootstrapEnd > bootstrapStart,
		"bootstrap step must precede first-release validation",
	);
	assert.match(
		bootstrapStep,
...
test("release skips repository mutation safely when App credentials are missing", async () => {
	...
	assert.match(
		release,
		/no repository mutation will be attempted with GITHUB_TOKEN/,
	);
	assert.match(
		release,
		/steps\.release_credentials\.outputs\.enabled == 'true'/,
	);
	assert.doesNotMatch(
		release,
...
test("release bootstrap keeps release App permissions minimal and scopes its private key", async () => {
	const release = await source(".github/workflows/release.yml");
	const jobEnv = release.slice(
		release.indexOf("    env:"),
		release.indexOf("    steps:"),
	);
...
test("release normalizes package-lock metadata before semantic-release", async () => {
	...
	assert.ok(
		normalizePosition >= 0,
		"release workflow must normalize npm lock metadata",
	);
	assert.ok(
		semanticPosition > normalizePosition,
		"lock metadata must be normalized before semantic-release",
	);
	assert.match(
		release,
