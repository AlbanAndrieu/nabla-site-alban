...
test("pre-commit keeps one auto-fixing Ruff lint authority before Ruff format", async () => {
	const config = await readFile(join(repositoryRoot, ".pre-commit-config.yaml"), "utf8");
	...
	const ruffBlock = config.slice(start, nextRepository === -1 ? undefined : nextRepository);
	...
	assert.ok(lintPosition < formatPosition, "ruff-check must auto-fix before ruff-format");
