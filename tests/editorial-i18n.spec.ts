...
test.describe("editorial page localization", () => {
  ...
  test("TrueNAS exposes a localized primary heading", async ({ page }) => {
    ...
    await expect(
      page.getByRole("link", { name: "Ouvrir le projet Nabla" }),
    ).toHaveAttribute("href", "/fr/nabla.html");
    await expect(
  ...
  test("the email page renders structured mail links", async ({ page }) => {
    ...
    await expect(
      page.getByRole("link", { name: "invoice@albandrieu.com", exact: true }),
    ).toHaveCount(2);
  });

  test("CV and contact use localized accessible hero links", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("link", { name: /Me contacter/ }),
    ).toHaveAttribute("href", "/fr/contact.html");

    await page.goto("/fr/contact.html");
    await expect(
      page.getByRole("link", { name: /Voir mon CV/ }),
  ...
  test("Jus Mundi KPIs are localized in French", async ({ page }) => {
    ...
    await expect(
      page.getByText("Tâches réalisées", { exact: true }),
    ).toBeVisible();
    await expect(
