...
export default function HomelabStatusOverview({
	...
	const refreshInProgress =
		healthRefreshing && (healthUnavailable || snapshot === null);
	...
	return (
		<>
			{(truenasDown || truenasWarning) && (
				<div
					className={`${statusStyles.surface} ${truenasDown ? statusStyles.fail : statusStyles.warn}`}
					role="alert"
					data-truenas-connectivity-warning
				>
					<strong>
			...
			{refreshInProgress && (
				<div
					className={`${statusStyles.surface} ${statusStyles.info}`}
					role="status"
					data-homelab-health-refreshing
				>
					<strong>
						<i className="fas fa-rotate fa-spin" aria-hidden="true" />{" "}
						{t("truenas.refreshing")}
					</strong>
					{" — "}
					{t("truenas.refreshingNote")}
				</div>
			)}

			{runtimeObservationIncomplete && (
				<div
					className={`${statusStyles.surface} ${statusStyles.warn}`}
					role="status"
			...
			<div className={styles.statusLegend} data-homelab-status-legend>
				<div
				...
				>
					...
					<a
						...
						href="#critical-dependency-hierarchy"
						onClick={(event) => {
