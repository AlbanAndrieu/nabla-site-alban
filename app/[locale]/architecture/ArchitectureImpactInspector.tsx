...
function stateClass(state?: string): string {
	...
	if (state === "fail") return styles.stateFail;
	if (state === "warn") return styles.stateWarn;
	...
	switch (cause.code) {
		...
		case "runtime_degraded":
			return french
				...
				: "TrueNAS runtime degraded or transitional";
		case "healthy":
			return french
				? "Preuves locales et dépendances saines"
				: "Healthy local and dependency evidence";
		case "unknown":
...
export default function ArchitectureImpactInspector({
	...
	const impacts = useMemo(
		() => (selectedId ? affectedDependents(topology, selectedId) : []),
		[topology, selectedId],
	);
			...
			<div className="container py-4">
				<div className={styles.panel}>
					<div className={styles.headingRow}>
						...
						<div>
							<h2 id="service-impact-inspector-title" className="h4 mb-1">
								{french
					...
					>
						<option value="">
							{french ? "Choisir un service…" : "Choose a service…"}
						</option>
						{nodes.map((node) => (
					...
					{selectedId ? (
						<div className={styles.result} data-selected-service-impact>
							<div className="d-flex flex-wrap gap-2 align-items-center justify-content-between">
								<strong>{names.get(selectedId) ?? selectedId}</strong>
								<a href={`/${locale}/truenas#service-${selectedId}`}>
									{french
										? "Ouvrir la carte TrueNAS"
										: "Open TrueNAS service card"}
								</a>
							</div>
							{entry ? (
								<>
									...
									<ul className="ps-3 mb-2" data-architecture-root-cause>
										{causes.map((cause, index) => (
											<li
												...
												className={stateClass(cause.state)}
											>
								...
								<p className={styles.muted}>
									{french
										? "Aucune preuve de santé courante pour ce nœud."
										: "No current health evidence for this node."}
								</p>
							)}
							...
							>
								{showImpact
									? french
										? "Masquer le rayon d’impact"
										: "Hide affected dependents"
									: french
										? "Afficher les dépendants impactables"
										: "Show affected dependents"}{" "}
								({impacts.length})
							</button>

							{showImpact ? (
								<div className={styles.impact} data-architecture-blast-radius>
									<ul className="mb-1 ps-3">
										{impacts.map((impact) => (
											<li key={impact.id}>
												<a
													href={`#service-${impact.id}`}
													onClick={() => selectService(impact.id)}
												>
													{impact.name}
												</a>{" "}
									...
									<p className={styles.muted}>
										{french
											? "Rayon d’impact structurel : ces services dépendent du nœud sélectionné, mais ils ne sont pas nécessairement en panne maintenant."
											: "Structural blast radius: these services depend on the selected node, but they are not necessarily failing now."}
									</p>
								</div>
							) : null}
						</div>
