...
export function probeScopeMetricRows(
	...
	push(
		rows,
		french ? "Échantillonnage rotatif" : "Rotating sample",
		scope.rotating_sample,
	);
	...
	push(
		rows,
		french ? "Concurrence max" : "Max concurrency",
		scope.max_concurrency,
	);
	push(
		rows,
		french ? "Durée du lot" : "Batch elapsed",
		scope.elapsed_ms,
		" ms",
	);
	...
	const budgetUtilization =
		finite(scope.elapsed_ms) &&
		finite(scope.budget_seconds) &&
		scope.budget_seconds > 0
	...
	push(
		rows,
		french ? "Budget consommé" : "Budget utilization",
		budgetUtilization,
	);
	...
	if (evidence) {
		...
		push(
			rows,
			french ? "Preuves en cache" : "Cached evidence",
			evidence.cached,
		);
		push(
			rows,
			french ? "Couverture des preuves" : "Evidence coverage",
			evidence.coverage_percent,
			"%",
		);
		push(
			rows,
			french ? "TTL des preuves" : "Evidence TTL",
		...
		push(
			rows,
			french ? "Part fraîche" : "Fresh ratio",
			percentage(evidence.fresh, evidence.known),
		);
		push(
			rows,
			french ? "Part cache" : "Cached ratio",
			percentage(evidence.cached, evidence.known),
		);
	}
	return rows;
}
...
export function probeCoordinatorMetricRows(
	...
	push(
		rows,
		french ? "Refresh agrégé" : "Aggregate refresh",
	...
	push(
		...
		snapshot.probe_summary?.catalog_service_count,
	);
	push(
		rows,
		french ? "Stratégie sampling" : "Sampling strategy",
		snapshot.probe_summary?.sampling?.strategy,
	);
	push(
		rows,
		french ? "TTL cache sampling" : "Sampling cache TTL",
		snapshot.probe_summary?.sampling?.cache_ttl_seconds,
		"s",
	);
	push(
		rows,
		french ? "Source cache" : "Probe cache source",
		snapshot.probe_cache?.source,
	);
	push(
		rows,
		french ? "Âge cache" : "Probe cache age",
		snapshot.probe_cache?.age_seconds,
		"s",
	);
	push(
		rows,
		french ? "TTL cache" : "Probe cache TTL",
		snapshot.probe_cache?.ttl_seconds,
		"s",
	);
	push(
		rows,
		french ? "Cache stale" : "Probe cache stale",
		snapshot.probe_cache?.stale,
	);
	push(
		rows,
		french ? "Health-board" : "Health board",
		snapshot.health_board?.state,
	);
	push(
		rows,
		french ? "Health-board refresh" : "Health board refreshing",
		snapshot.health_board?.refreshing,
	);
	push(
		rows,
		french ? "Health-board généré" : "Health board generated",
		snapshot.health_board?.generated_at,
	);
	push(
		rows,
		french ? "Âge health-board" : "Health board age",
		snapshot.health_board?.age_seconds,
		"s",
	);
	push(rows, "Retry after", snapshot.health_board?.retry_after_seconds, "s");
	push(
		rows,
		french ? "Erreur health-board" : "Health board error",
		snapshot.health_board?.error,
	);
	push(
		rows,
		french ? "Lectures provider réutilisées" : "Provider reads reused",
		snapshot.reconciliation?.provider_reads_reused,
	);
	push(
		rows,
		french ? "Source runtime TrueNAS" : "TrueNAS runtime source",
