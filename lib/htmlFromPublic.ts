...
function decodeBasicEntities(text: string): string {
	return text.replace(
		/&(?:amp|quot|#39|lt|gt);/g,
		(entity) => BASIC_HTML_ENTITIES[entity] ?? entity,
	);
}
...
function withLocalePrefix(pathname: string, locale?: string): string {
	if (/^\/(en|fr)(\/|$)/.test(pathname)) return pathname;
...
function rewriteOneHref(
	...
	if (!pathPart.endsWith(".html")) return `href=${quote}${raw}${quote}`;
	pathPart = `/${pathPart.replace(/^\/+/, "")}`;
	pathPart = pathPart.replace(/\/index\.html$/i, "").replace(/\.html$/i, "");
	return `href=${quote}${withLocalePrefix(pathPart || "/", locale)}${query}${hash}${quote}`;
}
...
export function rewriteLegacyHtmlHrefs(
	...
	return out.replace(/\bsrc='([^']*)'/gi, (_full, raw: string) =>
		rewriteOneSrc("'", raw),
	);
}
...
export function preserveCanonicalProductNames(
	fragment: string,
	file: string,
	locale?: string,
): string {
	if (normalizeLocale(locale) !== "fr" || file.startsWith("cv/"))
		return fragment;
	let out = fragment;
	for (const [translated, canonical] of FRENCH_PRODUCT_NAME_REPAIRS)
		out = out.replaceAll(translated, canonical);
	return out;
}
...
export function removeLegacyElementsById(
	fragment: string,
	ids: readonly string[],
): string {
	let out = fragment;
	for (const id of ids) {
		...
		const pattern = new RegExp(
			`<((?:section|article))\\b(?=[^>]*\\bid=["']${escaped}["'])[^>]*>[\\s\\S]*?<\\/\\1>`,
			"gi",
		);
...
export async function loadPublicHtmlFragment(
	...
	fragment = preserveCanonicalProductNames(fragment, file, locale);
